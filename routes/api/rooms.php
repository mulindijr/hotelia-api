...
use App\Http\Controllers\Api\V1\Rooms\AvailabilityController;
...
Route::middleware(['auth:sanctum'])->group(function () {
    ...
    Route::prefix('amenities')->group(function () {
        ...
        Route::match(['put', 'patch'], '/{amenity}', [AmenityController::class, 'update'])->middleware('permission:'.Permissions::UPDATE_AMENITIES)->name('amenities.update');
    ...
    Route::prefix('hotels/{hotel}/room-types')->scopeBindings()->group(function () {
        ...
        Route::match(['put', 'patch'], '/{roomType}', [RoomTypeController::class, 'update'])->middleware('permission:'.Permissions::UPDATE_ROOM_TYPES)->name('room-types.update');
    ...
    Route::prefix('hotels/{hotel}/rooms')->scopeBindings()->group(function () {
        ...
        Route::match(['put', 'patch'], '/{room}', [RoomController::class, 'update'])->middleware('permission:'.Permissions::UPDATE_ROOMS)->name('rooms.update');
        Route::delete('/{room}', [RoomController::class, 'destroy'])->middleware('permission:'.Permissions::DELETE_ROOMS)->name('rooms.destroy');
    });

    // 4. Availability Matrix Route under Hotels
